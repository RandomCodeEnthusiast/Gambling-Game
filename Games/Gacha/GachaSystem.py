def Gacha(Player : int,WishCount : int) -> None :
    ...
    from Data.InventoryManagement.BackUpSystem.BackUp import FileBackUp,FileElementReplacer
    from Data.InventoryManagement.StoneManagement.Stone import StoneModifier,StoneCheck

    #Spends the inputed amount of stones
    StoneCount = StoneCheck(Player)

    if StoneCount >= WishCount :
        StoneModifier(WishCount,Player,Spend=True)
    else :
        print(f'You are missing {WishCount - StoneCount}')
        return None
